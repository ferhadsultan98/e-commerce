...
const BottomHeader = ({ onAddToWishlist, onAddToCart }) => {
  const [wishlistItems, setWishlistItems] = useState([]); // Track wishlist products
  const [cartItems, setCartItems] = useState([]); // Track cart products
  ...
  const handleAddToWishlist = (product) => {
    ...
    if (!wishlistItems.some((item) => item.id === product.id)) {
      ...
      onAddToWishlist(product); // Call parent handler if needed
    }
  };
  ...
  const handleAddToCart = (product) => {
    ...
    if (!cartItems.some((item) => item.id === product.id)) {
      ...
      onAddToCart(product); // Call parent handler if needed
    }
  };
