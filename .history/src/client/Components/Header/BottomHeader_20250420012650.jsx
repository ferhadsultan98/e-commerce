                    ...
                    productData.subCategories[hoveredCategory] ? (
                      <div className="subCategoryContainer">
                        ...
                        <div className="subCategories">
                          {productData.subCategories[hoveredCategory].map(
                            (sub, index) => (
                              <Link
                                ...
                                className={`subCategoryItem ${
                                  sub.featured ? "featuredSubcategory" : ""
                                }`}
                              >
                                {sub.name}
                              </Link>
      ...
      <div className={`mobileMenu ${mobileMenuOpen ? "open" : ""}`}>
        ...
        <div className="mobileMenuContent">
          <div className="mobileCategoryList">
            <h3>Categories</h3>
            {categories.map((category) => (
              <div key={category.id} className="mobileCategoryItem">
                <Link to={`/category/${category.id}`}>
                  {category.icon}
                  <span>{category.name}</span>
                </Link>
                {productData.subCategories[category.id] && (
                  <div className="mobileSubCategories">
                    {productData.subCategories[category.id].map((sub, index) => (
                      <Link
                        to={`/category/${category.id}/${sub.name
                          .toLowerCase()
                          .replace(/\s+/g, "-")}`}
                        key={index}
                        className="mobileSubCategoryItem"
                      >
                        {sub.name}
                      </Link>
                    ))}
