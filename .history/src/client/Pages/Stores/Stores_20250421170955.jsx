...
const Stores = () => {
  ...
  const [stores, setStores] = useState([
    {
      ...
      location: { lat: null, lng: null } // Initialize with null
    },
    {
      id: 2,
      name: 'Gəncə Şubesi',
      ...
      const updatedStores = await Promise.all(
        stores.map(async (store) => {
          if (!store.location.lat || !store.location.lng) {
            ...
            return {
              ...store,
              location: coords || { lat: 40.4093, lng: 47.5769 } // Fallback to Azerbaijan center
            };
          }
          return store;
        })
  ...
  const mapCenter = selectedStore ? [selectedStore.location.lat, selectedStore.location.lng] : azerbaijanCenter;
        ...
        <div className="storeMapContainer">
          ...
          <div className="storeMapWrapper">
            ...
            >
              ...
              {stores
                .filter((store) => store.location.lat && store.location.lng) // Only render markers with valid coordinates
                .map((store) => (
                  <Marker
                    key={store.id}
