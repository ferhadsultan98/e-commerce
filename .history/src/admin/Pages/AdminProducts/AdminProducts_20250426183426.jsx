...
const AdminProducts = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const subCategoriesSnap = await get(ref(db, "subCategories"));

        const productsData = productsSnap.exists() ? productsSnap.val() : [];
        const categoriesData = categoriesSnap.exists() ? categoriesSnap.val() : [];
        const subCategoriesData = subCategoriesSnap.exists() ? subCategoriesSnap.val() : {};
  ...
  useEffect(() => {
    if (selectedCategory !== "all" && subCategoriesMap[selectedCategory]) {
      ...
      <div className="tableContainer">
        {isLoading ? (
          ...
          <table className="productsTable">
            <thead>
              <tr>
                ...
                <th onClick={() => handleSort('oldPrice')}>Old Price (₼) {getSortIcon('oldPrice')}</th>
                <th onClick={() => handleSort('discount')}>Discount (%) {getSortIcon('discount')}</th>
                <th>Image</th>
              </tr>
            </thead>
            <tbody>
          ...
          <div className="noResultsContainer">
            <p className="noResultsMessage">No products found with the current filters.</p>
            <button className="clearFiltersBtn" onClick={clearFilters}>
              Clear Filters
            </button>
          </div>
